# AQA-Bench Evaluation

The eval harness is adapted from the [TIME framework](https://github.com/zqiao11/TIME). Everything runs from the `benchmark/` directory.
...
```yaml
time_repo: .                        # path to the benchmark directory (contains experiments/)
...
The `short` horizon is the only one currently configured. `test_length: 17376` corresponds to approximately 2 years of hourly data minus one context length window.

---

## Setup

Install all dependencies from the project root, then run eval scripts from `benchmark/`:

```bash
# from the project root
uv sync           # installs base + eval deps into air-quality-arena/.venv
source .venv/bin/activate     # activate before running anything in benchmark/

cd benchmark
```

> `run.py` looks for `.venv` inside `benchmark/`. Since the venv is at the project root, activate it first so that `python` and `uv pip install` both resolve to the right environment.
...
```bash
# from benchmark/ (with venv activated)
...
```
benchmark/output/results/<model_name>/<dataset_id>/<horizon>/
